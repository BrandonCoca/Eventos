...
class EditRegistro extends EditRecord
{
    ...
    protected function afterSave(): void
    {
        ...
        try {
            ...
            DB::rollBack();
            Log::error('Error al editar el registro: ' . $e->getMessage(), [
                'exception' => $e,
                'user_id' => Auth::id(),
            ]);
    ...
    private function sendSuccessNotification(): void
    {
        Notification::make()
            ...
            ->body('El registro ha sido editado exitosamente')
            ->success()
            ->send();
    }
    private function sendErrorNotification(): void
    {
        Notification::make()
            ...
            ->body('Ocurrió un error al editar el registro, vuelve a intentarlo. Si el problema persiste, contacta al administrador.')
            ->danger()
            ->send();
    }
    ...
    protected function handleException($exception): void
    {
        if ($exception instanceof \Illuminate\Validation\ValidationException) {
            ...
            $this->notify(
                Notification::make()
                    ->title('Error')
                    ->body('Ha ocurrido un error al editar el registro. Contacta al administrador.')
                    ->danger()
                    ->send()
            );
        }
