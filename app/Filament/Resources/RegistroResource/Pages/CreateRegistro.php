...
class CreateRegistro extends CreateRecord
{
    ...
    private function sendErrorNotification(): void
    {
        Notification::make()
            ...
            ->body('Ocurrió un error al crear el registro, vuelve a intentarlo. Si el problema persiste, contacta al administrador.')
            ->danger()
            ->send();
    }
    ...
    protected function handleException($exception): void
    {
        if ($exception instanceof ValidationException) {
            ...
            $this->notify(
                Notification::make()
                    ->title('Error')
                    ->body('Ha ocurrido un error al crear el registro. Contacta al administrador.')
                    ->danger()
                    ->send()
            );
        }
