...
class EditInscripcion extends EditRecord
{
    ...
    protected function afterSave(): void
    {
        ...
        try {
            ...
            DB::rollBack();
            Log::error('Error al editar la inscripcion: ' . $e->getMessage(), [
                'exception' => $e,
                'user_id' => Auth::id(),
            ]);
    ...
    {
        Notification::make()
            ->title('Inscripcion editada')
            ->icon('heroicon-o-check-circle')
            ->body('La inscripcion ha sido editada exitosamente')
            ->success()
            ->send();
    }
    ...
    {
        Notification::make()
            ->title('Error al editar la inscripcion')
            ->icon('heroicon-o-x-circle')
            ->body('Ocurrió un error al editar la inscripcion, vuelve a intentarlo. Si el problema persiste, contacta al administrador.')
            ->danger()
            ->send();
    }
    ...
    protected function handleException($exception): void
    {
        if ($exception instanceof \Illuminate\Validation\ValidationException) {
            ...
            $this->notify(
                Notification::make()
                    ->title('Error')
                    ->body('Ha ocurrido un error al editar la inscripcion. Contacta al administrador.')
                    ->danger()
                    ->send()
            );
        }
