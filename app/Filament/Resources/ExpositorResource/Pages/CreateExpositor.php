...
class CreateExpositor extends CreateRecord
{
    ...
    {
        Notification::make()
            ->title('Expositor creado')
            ->icon('heroicon-o-check-circle')
            ->body('El expositor ha sido creado exitosamente')
            ->success()
            ->send();
    }
    private function sendErrorNotification(): void
    {
        Notification::make()
            ...
            ->body('Ocurrió un error al crear el expositor, vuelve a intentarlo. Si el problema persiste, contacta al administrador.')
            ->danger()
            ->send();
    }
    ...
    protected function handleException($exception): void
    {
        if ($exception instanceof ValidationException) {
            ...
            $this->notify(
                Notification::make()
                    ->title('Error')
                    ->body('Ha ocurrido un error al crear el expositor. Contacta al administrador.')
                    ->danger()
                    ->send()
            );
        }
